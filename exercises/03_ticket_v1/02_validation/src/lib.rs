...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        // the tests require these panic strings to pass
        if title.is_empty() {
        ...
        // define a static array of status strings
        let valid_status: [&str; 3] = ["To-Do", "In Progress", "Done"];
        if !valid_status.contains(&status.as_str()) {
            panic!("Only `To-Do`, `In Progress`, and `Done` statuses are allowed")
        }
        
        // after ticket parameters are valididated, create instance of Ticket
        Self {
            title,
            description,
            status,
