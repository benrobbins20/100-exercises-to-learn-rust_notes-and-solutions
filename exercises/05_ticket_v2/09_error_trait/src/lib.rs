...
// we don't care about the messages that are encapped in 
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        // e is now of type Error, so don't need to wrap each ticketnewerror in Err
        Err(e) => match e {

            /* 
                the strings (_) that are produced inside the tuple variants are ignored
                `e` contains the error enum(String), just defer to TicketErrorEnum Display impl to 
                print the string 
            */ 
